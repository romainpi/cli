...
export default class AppGenerateExtension extends Command {
  static description = 'Generate an Extension'
  ...
  public async run(): Promise<void> {
    ...
    const promptAnswers = await generateExtensionPrompt({
      ...
      extensionTypesAlreadyAtQuota: this.limitedExtensionsAlreadyGenerated(app),
      name: flags.name,
  ...
  /**
   * If the type passed as flag is not valid because it has already been generated
  ...
  validateExtensionTypeLimit(app: AppInterface, type: string | undefined) {
    if (type && this.limitedExtensionsAlreadyGenerated(app).includes(type)) {
      throw new error.Abort(
   ...
   * Use this method to retrieve a list of the limited types that have already been generated
   *
   * @param app {AppInterface} current App
   * @returns {string[]} list of extensions that are limited by quantity and are already generated
   */
  limitedExtensionsAlreadyGenerated(app: AppInterface): string[] {
