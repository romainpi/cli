...
import {createDeployment, healthCheck, TooManyRequestsError, UnrecoverableError, uploadDeployment} from './upload.js'
...
describe('createDeploymentStep()', () => {
  it('makes a call to DMS to create the deployment', async () => {
  ...
  describe('failure', () => {
    ...
    it('throws a generic error if we are getting a user error', async () => {
      ...
      vi.mocked(api.oxygen.request).mockResolvedValue(dmsResponse)
...
describe('uploadDeploymentStep()', async () => {
  describe('success', async () => {
    it('makes a call to DMS to upload the deployment files', async () => {
      ...
      const mockedUploadDeployment = vi
        .fn()
        .mockResolvedValue({json: vi.fn().mockResolvedValue(dmsResponse), status: 200})
      ...
      expect(api.oxygen.uploadDeploymentFile).toHaveBeenCalledWith('dms.address', '123', formData)
    })
  })
  ...
  describe('failure', async () => {
    ...
    it('throws an exception if DMS returns a non-200 status code', async () => {
      const dmsResponse: GraphQLError = {
      ...
      const mockedUploadDeployment = vi
        .fn()
        .mockResolvedValue({json: vi.fn().mockResolvedValue(dmsResponse), status: 500})
      vi.mocked<any>(api.oxygen.uploadDeploymentFile).mockImplementation(mockedUploadDeployment)
      const tmpDir = 'tmp/dir'
      vi.mocked<any>(file.inTemporaryDirectory).mockImplementation(async (runner: (tmpDir: string) => Promise<void>) =>
        runner(tmpDir),
    ...
    it('throws an exception if DMS returns a user error', async () => {
      const dmsResponse = {
        data: {
          uploadDeployment: {
            deployment: {
    ...
    it('throws an ratelimit exception if being ratelimited by DMS', async () => {
      const mockedUploadDeployment = vi
        .fn()
