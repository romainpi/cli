...
const dmsAddress = 'oxygen-dms.shopifycloud.com'
...
describe('oxygen-api', () => {
  test('calls the graphql client once', async () => {
    ...
    await oxygenApi.request(dmsAddress, 'query', mockedToken, {some: 'variables'})
  ...
  test('request is called with the correct parameters', async () => {
    ...
    await oxygenApi.request(dmsAddress, 'query', mockedToken, {variables: 'variables'})
  ...
  test('buildHeaders is called with the deployment token', async () => {
    ...
    await oxygenApi.request(dmsAddress, 'query', mockedToken, {})
...
describe('uploadDeploymentFile', () => {
  test('makes a post request to DMS using the provided form data', async () => {
    const responseBody = {
      data: {
        uploadDeployment: {
          deployment: {
    ...
    const gotResponse = await oxygenApi.uploadDeploymentFile(dmsAddress, mockedToken, mockedFormData as any)
    expect(shopifyFetch).toBeCalledWith('dms', `https://${dmsAddress}/api/graphql/deploy/v1`, {
      method: 'POST',
      body: mockedFormData,
      headers,
    })
